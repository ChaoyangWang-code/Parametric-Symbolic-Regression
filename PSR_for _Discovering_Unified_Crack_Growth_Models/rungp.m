...
for run=1:gp.runcontrol.runs
    ...
    for count=1:gp.runcontrol.num_gen
        ...
        if count == 1
            ...
            tic
            %gp = initbuild(gp);
            gp = initbuild_d(gp);           %Generate different pops.
            %gp = initbuild_copy(gp);
            toc
            pop_all=gp.pop;
            positions=zeros(gp.runcontrol.pop_size,1);
            gp.state.new_pop_num=gp.runcontrol.pop_size;        
        else
            ...
            gp = find_new_pop(gp);
            parallelTime = toc;  
            fprintf('Time taken to generate new pops.: %.3f s\n', parallelTime);
        end

        %calculate fitnesses of population members
        gp.fitness.values_alldata=[];

        %gp = evalfitness(gp,count);
        gp = evalfitness_par(gp,count);
        gp.state.uniqueCount=pop_unique_num(gp);
        ...
        if gp.fitness.plot_every_gen
            MULTI_R_show_results_gen_i(gp,count);
        end
