...
for ui=1:pu_num
    ...
    matches = ismember(p_all, row_to_find, 'rows');
    [row, ~] = find(matches);
    elite_tour_ind(ui,1)=tour_ind(row(1),1);
end
...
newPop_skim1 = cell(num2skim1,1);
%
for g=1:num2skim1
    elite_of_all_pop_id=elite_tour_ind(g);
    newPop_skim1{g,1} = gp.pop{elite_of_all_pop_id,1};
    loss_elite_gen(g,1)=gp.fitness.values(elite_of_all_pop_id);
end
gp.fitness.loss_elite_gen=loss_elite_gen;
gp.fitness.pop_elite_gen=newPop_skim1;

entendactor=gp.fitness.entendactor;

for g=1:num2skim2  
    Index_temp = elite_tour_ind(g);    
    %for the first individual to skim off, pick the 'best' member with the
    %lowest complexity
    if 1
        bestInds = find( (  gp.fitness.values <=entendactor(2)*(gp.fitness.values(Index_temp))  )&( gp.fitness.values >=entendactor(1)*(gp.fitness.values(Index_temp)) ));
        [~,bestInds_less_complexity] = sort(gp.fitness.complexity(bestInds));                 
        oldIndex(g,1) = bestInds(bestInds_less_complexity(1)); %if more than one with same complexity just pick the first
    end   
end

oldIndex_u=unique(oldIndex,'rows','stable');            
num2skim2=size(oldIndex_u,1);                                  
newPop_skim2 = cell(num2skim2,1);
for g=1:num2skim2

    %newIndex = g+num2skim1;
    newIndex = g;
    copiedIndividual = gp.pop{oldIndex_u,1};
    
    %cache fitness, complexity etc.
    if gp.runcontrol.usecache
        cachedData.complexity = gp.fitness.complexity(oldIndex_u,1);
        cachedData.returnvalues = gp.fitness.returnvalues{oldIndex_u,1};
        cachedData.value = gp.fitness.values(oldIndex_u,1);
        gp.fitness.cache(newIndex) = cachedData;
    end   

    newPop_skim2{newIndex,1} = copiedIndividual;
end

num2skim=num2skim1+num2skim2;
newPop_skim=vertcat(newPop_skim1, newPop_skim2);
...
%% 
parfor i=1:gp.runcontrol.pop_size
    tempgp = gp;
    num_genes=size(newPop{i,1},2);
    if num_genes>1
        pop_i_no_same_gene=delete_same_gene(newPop{i,1},num_genes,tempgp);
        newPop{i,1}=pop_i_no_same_gene;  
    end
end
gp.pop = newPop;
