function treestr = treegen(gp,ERC_number_of_last_genes,fixedDepth)
...
current_ERC_number=ERC_number_of_last_genes;
    ...
    elseif nodeType == 2 %pick a terminal (or an arity 0 function, if active)
        ...
        if fun_lengthareq0 && (rand >= 0.5 || (num_inp==0 && p_ERC==0))
            ...
            if term %then use an ordinary terminal not a constant
                ...
                %ERCgenerated = true;             
                %%%20231010
                if (current_ERC_number+1)<=max_number_of_ERC
                    ERC_Choice=current_ERC_number+1;
                    treestr = strrep(treestr,'@',['c' sprintf('%d',ERC_Choice)]);
                    current_ERC_number=current_ERC_number+1;
                else
                    inpChoice = ceil(rand * num_inp);                               
                    treestr = strrep(treestr,'@',['x' sprintf('%d',inpChoice)]);
                end                
