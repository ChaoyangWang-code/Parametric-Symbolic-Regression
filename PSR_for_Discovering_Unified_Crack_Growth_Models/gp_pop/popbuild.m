...
max_number_of_ERC=gp.nodes.inputs.max_number_of_ERC;
%reset cache
...
while buildCount < num2build
    ...
    if eventType == 1
        ...
        [newPop_temp,check_index] = orderconst(parent,max_number_of_ERC);
        if check_index==1
            newPop{buildCount,1}=newPop_temp;
        else
            buildCount=buildCount-1;
        end
    ...
    elseif eventType == 3
        ...
        if highLevelCross
            if numMumGenes>1 || numDadGenes>1
                ...
                %buildCount = buildCount+1;
                [newPop_temp,check_index] = orderconst(mumOffspring(1:(min(end,maxGenes))),max_number_of_ERC);
                if check_index==1
                    newPop{buildCount,1}=newPop_temp;
                    buildCount = buildCount+1;
%                 else
%                     buildCount=buildCount-1;
                ...
                if buildCount <= num2build
                    ...
                    [newPop_temp,check_index] = orderconst(dadOffspring(1:(min(end,maxGenes))),max_number_of_ERC);
                    if check_index==1
                        newPop{buildCount,1}=newPop_temp;
                    else
                        buildCount=buildCount-1;
        ...
        if ~highLevelCross
            ...
            %buildCount = buildCount+1;
            [newPop_temp,check_index] = orderconst(dad,max_number_of_ERC);
            if check_index==1
                newPop{buildCount,1}=newPop_temp;
                buildCount = buildCount+1;
%         else
%             buildCount=buildCount-1;
            ...
            if buildCount <= num2build               
                ...
                [newPop_temp,check_index] = orderconst(mum,max_number_of_ERC);
                if check_index==1
                    newPop{buildCount,1}=newPop_temp;
                else
                    buildCount=buildCount-1;
