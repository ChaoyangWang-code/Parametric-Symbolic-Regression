...
train_results=returnvals_validation{pop_choose,1}{1,1}(1:10,:);
loss_MSE=max(train_results(:,1));
gene=genes(pop_choose,1);
con=cons(pop_choose,1);
theta=train_results(:,(4+con):(4+con+gene+1));
var_temp=var(theta);
loss_VAR=max(var_temp);
loss_num=gene+2;
fitness_train(pop_choose,1)=loss_MSE;
fitness_train(pop_choose,2)=loss_VAR;
fitness_train(pop_choose,3)=loss_num;

validation_results=returnvals_validation{pop_choose,1}{1,1}(11:13,:);
...
theta_train=theta_all(1:10,:);
theta_validation=theta_all(11:13,:);
